...
from utils import begin_log_section, log_run, load_model, save_model, score_with_metric
...
class SklearnRunner:
  ...
  def run_cv(self, name: str = '', params: dict | None = None) -> dict:
    ...
    steps = (
      pipeline.regressor.named_steps
      if self.config.data.transform_target
      else pipeline.named_steps
    )
    model = steps['model']
  ...
  def fit_full(self, name: str = '', params: dict | None = None):
    ...
    val_metric = score_with_metric(pipeline, X_val, y_val, self.config.metric)

    elapsed_s = time.perf_counter() - start
    steps = (
      pipeline.regressor_.named_steps
      if self.config.data.transform_target
      else pipeline.named_steps
    )
    model = steps['model']
