...
from utils import inverse_transform_target, log_message
...
def _get_predictions(outputs: torch.Tensor) -> torch.Tensor:
  if outputs.ndim == 1 or outputs.shape[-1] == 1:
    return outputs if outputs.ndim == 1 else outputs.squeeze(-1)
...
def evaluate(
  ...
  *,
  ...
  if transform_target:
    y_true = inverse_transform_target(y_true)
    y_pred = inverse_transform_target(y_pred)
  ...
  *,
  ...
  for epoch in range(start_epoch, num_epochs + 1):
    ...
    last_val_loss, val_metric = evaluate(
      ...
      device,
      metric_name,
      transform_target=bool(config is not None and config.data.transform_target),
    )
