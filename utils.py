...
def transform_target(target):
  """Apply some target transform used during training."""
  return np.log1p(target) 


def inverse_transform_target(y_pred):
  """Invert log1p target transform for predictions."""
...
def select_best_cv_result(results: list[dict], config) -> dict:
  ...
  best_selector = max if config.is_negative_metric else min
...
def write_result_md(results: list[dict], config) -> Path:
  ...
  sorted_results = sorted(
    results,
    key=lambda result: result['metric'],
    reverse=config.is_negative_metric,
  )
